...
def apply_soft_mastering(file_path, weight, amp, pitch_val):
    """इको को कम करके आवाज़ को साफ़ बनाना"""
    ...
    # ✅ इको फिक्स: वॉल्यूम कम और डिले बढ़ा दिया ताकि साफ़ सुनाई दे
    # यहाँ इको को -18dB कम कर दिया है (पहले -12dB था)
    echo = sound - 18 
    sound = sound.overlay(echo, position=180) 
    
    # भारीपन के लिए स्मूथ फ़िल्टर
    sound = sound.low_pass_filter(3800)
    
    final_path = "shriram_perfect_balance.wav"
...
def generate_voice(text, voice_sample, speed, human_feel, weight, amp, pitch_val, progress=gr.Progress()):
    ...
    for i, chunk in enumerate(chunks):
        progress((i+1)/len(chunks), desc=f"🚀 परफेक्ट क्लोनिंग: {i+1}/{len(chunks)}")
        ...
        tts.tts_to_file(
            ...
            temperature=human_feel, top_p=0.82, gpt_cond_len=8
        )
    ...
    return apply_soft_mastering("combined.wav", weight, amp, pitch_val)

# 🎨 अपडेटेड UI
with gr.Blocks(theme=gr.themes.Soft(primary_hue="orange")) as demo:
    gr.Markdown("# 🚩 श्रीराम वाणी - 100% परफेक्ट बैलेंस")
    ...
    out = gr.Audio(label="फाइनल साफ़ आवाज़", type="filepath", autoplay=True)
